...
define( 'BP_AVATAR_DEFAULT', 'http://tamrielfoundry.com/wp-content/themes/apocrypha/images/avatars/undecided-large.jpg' );
define( 'BP_AVATAR_DEFAULT_THUMB', 'http://tamrielfoundry.com/wp-content/themes/apocrypha/images/avatars/undecided.jpg' );
...
function foundry_profile_tabs() {
	...
	// Main navigation
	$bp->bp_nav['profile']['name'] 		= 'User Profile';
	$bp->bp_nav['profile']['position'] 	= 1;
	$bp->bp_nav['activity']['name'] 	= 'Recent Activity';
	$bp->bp_nav['activity']['position'] = 2;
	$bp->bp_nav['forums']['name'] 		= 'Forum Tracker';
	$bp->bp_nav['forums']['position'] 	= 3;
	$bp->bp_nav['friends']['position'] 	= 4;
	$bp->bp_nav['groups']['position'] 	= 5;
	$bp->bp_nav['messages']['position'] = 6;
	$bp->bp_nav['settings']['name'] 	= 'Account Settings';
	$bp->bp_nav['settings']['position'] = 100;
	
	// Profile sub-navigation
	$bp->bp_options_nav['activity']['just-me']['name'] 			= 'All Activity';
	$bp->bp_options_nav['profile']['public']['name'] 			= 'View User Profile';
	$bp->bp_options_nav['profile']['public']['link'] 			= $bp->displayed_user->domain . 'profile';
	...
	if ( bp_is_my_profile() ) {
		...
		if ( $friend_requests ) {
			$friend_plus = '<span class="notification-count">+' . $friend_requests . '</span>';
		...
		if ( $unread_messages ) {
			$message_plus = '<span class="notification-count">+' . $unread_messages . '</span>';
	...
	// Add in-game character screen
	bp_core_new_subnav_item( array(
		'name' 				=> 'In-Game Character',
		'slug' 				=> 'character',
		'parent_url' 		=> $bp->displayed_user->domain . $bp->profile->slug . '/',
		'parent_slug' 		=> $bp->profile->slug,
		'screen_function' 	=> 'apoc_game_characters_screen',
		'position' 			=> 10 ) );
...
/*
 * Create in-game character profile screen
 * @since 0.1
 */
function apoc_game_characters_screen() {
	bp_core_load_template( apply_filters( 'apoc_game_characters_template', 'members/single/profile/character' ) );
}
