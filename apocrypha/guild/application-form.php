...
if( isset( $_POST['submitted']) ) {
	...
	//Check fields
		...
		$character_race = $_POST['character-race'];
		...
		if( !isset($hasError) ) { // Send the email!
			...
			$body .= "<li>Character Race: $character_race</li>";
...
<form action="<?php the_permalink(); ?>" id="guild-application-form" method="post">
	<ol id="guild-application-list">	
		...
		<li class="select">
			<label for="character-race"><i class="icon-male icon-fixed-width"></i>Character Race (intended):</label>
			<select name="character-race">
			<?php $races = array ( 'altmer' , 'bosmer' , 'khajiit' ); 
				foreach ( $races as $key => $race ) :
				echo '<option value="' . $race . '" ' . selected( $_POST['character-race'] , $race , true ) . '>' . ucfirst($race) . '</option>';
			endforeach; ?>
			</select>
		</li>
		...
		<li class="form-field text">
			...
			<?php if($ageError != '') echo '<div class="error">' . $ageError . '</div>'; ?> 
		</li>
		...
		<li class="checkbox">
			...
			<ul class="radio-options-list">
				<li><input type="radio" name="playstyle" value="casual" <?php checked( $_POST['playstyle'] , 'casual' ); ?>/><label for="playstyle">0-13 Hours/Week</label></li>
				<li><input type="radio" name="playstyle" value="moderate" <?php checked( $_POST['playstyle'] , 'moderate' ); ?>/><label for="playstyle">14-27 Hours/Week</label></li>
				<li><input type="radio" name="playstyle" value="hardcore" <?php checked( $_POST['playstyle'] , 'hardcore' ); ?>/><label for="playstyle">28+ Hours/Week</label></li>
			</ul>
			<?php if($playstyleError != '') echo '<div class="error">' . $playstyleError . '</div>'; ?> 
		</li>
		...
		<li class="wp-editor">
			...
			<?php if($experienceError != '') echo '<div class="error">' . $experienceError . '</div>'; ?> 
			<?php // Load the TinyMCE Editor
			$thecontent = wpautop( stripslashes($_POST['experience']) );
		...
		<li class="wp-editor">
			...
			<?php if($youofferError != '') echo '<div class="error">' . $youofferError . '</div>'; ?> 
			<?php // Load the TinyMCE Editor
			$thecontent = wpautop( stripslashes($_POST['youoffer']) );
		...
		<li class="checkbox">
			...
			<input type="radio" name="otherguild" value="yes" <?php checked( $_POST['otherguild'] , 'yes' );?>/><label for="otherguild">Yes</label>
			<input type="radio" name="otherguild" value="no" <?php checked( $_POST['otherguild'] , 'no' );?>/><label for="otherguild">No</label>
			<?php if($otherguildError != '') echo '<div class="error">' . $otherguildError . '</div>'; ?> 
		</li>
		...
		<li class="text">		
			...
			<?php if($guildurlError != '') echo '<div class="error">' . $guildurlError . '</div>'; ?> 
		</li>
		...
		<li class="checkbox">
			...
			<?php if( isset($_POST['voicechat']) ) $voicechat = $_POST['voicechat']; ?>
			<input type="radio" name="voicechat" value="yes" <?php checked( $_POST['voicechat'] , 'yes' ); ?>/><label for="voicechat">Yes</label>
			<input type="radio" name="voicechat" value="no" <?php checked( $_POST['voicechat'] , 'no' ); ?>/><label for="voicechat">No</label>
			<?php if($voicechatError != '') echo '<div class="error">' . $voicechatError . '</div>'; ?> 
		</li>
		...
		<li class="wp-editor">
			...
			<?php // Load the TinyMCE Editor
			$thecontent = wpautop ( stripslashes($_POST['extrainfo']) );
		...
		<li class="honeypot">
			...
			<?php if($captchaError != '') echo '<div class="error">DIE ROBOT!</div>'; ?> 
		</li>
