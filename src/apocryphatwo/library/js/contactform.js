...
;$(document).ready(function(){
	$( "form#contact-form" ).submit( function( event ) {
		...
		if ( $( '#contact-notice' ).length == 0 ) {
			$(this).before('<div id="contact-notice" class="updated"></div>');
		}
		$( '#contact-notice' ).hide();
		...
		} else if ( !emails.test( data.email ) ) {
		...
		if ( error ) {
			$( '#contact-notice' ).addClass( 'error' ).text( error ).fadeIn();
		...
		// Otherwise, do some AJAX
			...
			$.ajax({
				...
				success	: function( data ) {
					...
					form.slideUp();
					$( '#contact-notice' ).removeClass('error').text( 'Contact submission successful, thank you for your comments!').fadeIn();
				},
